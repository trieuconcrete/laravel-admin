...
class PostController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $seoData = new SEO([
            ...
            'description' => 'Xem tất cả bài viết mới nhất tại Vận Tải Hoàng Phú Long',
            'keywords' => 'bài viết, vận tải, Hoàng Phú Long',
            'og:title' => 'Danh sách bài viết',
            'og:description' => 'Xem tất cả bài viết mới nhất tại Vận Tải Hoàng Phú Long',
            'og:type' => 'website',
        ]);
        ...
        return view('admin.posts.index', compact('posts', 'stats', 'categories', 'seoData'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Xử lý meta_tags
        $validated['meta_tags'] = collect(explode(',', $validated['meta_tags'] ?? ''))
            ->map(fn($tag) => trim($tag))
            ->filter()
            ->values()
            ->toArray();
        ...
        $post->seo()->updateOrCreate([], [
            ...
            'description' => $validated['meta_description'] ?? $validated['excerpt'],
            'keywords' => is_array($validated['meta_tags']) ? implode(',', $validated['meta_tags']) : $validated['meta_tags'] ?? null,
        ]);

        // Nếu là AJAX (Preview), trả về JSON
        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'post' => $post,
    ...
    public function update(Request $request, Post $post)
    {
        $validated = $request->validate([
            ...
            'meta_title' => 'nullable|string|max:60',
            'meta_description' => 'nullable|string|max:160',
            'meta_tags' => 'nullable|string|max:160',
            'published_at' => 'nullable|date'
        ]);
